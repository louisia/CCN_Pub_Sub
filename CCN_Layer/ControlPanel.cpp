...
ControlPanel::ControlPanel(DataPanel* pDataPanel, std::string localIP,
...
void ControlPanel::injectTransFaceTableToDataPanel() {
	...
	for (std::vector<AdjacentItem>::size_type i = 0;
			i != m_adjacentTable.size(); ++i) {
...
void ControlPanel::injectFIBToDataPanel() {
	...
	for (vector<RouterItem>::size_type i = 0; i != m_routerTable.size(); ++i) {
		...
		for (vector<RoleID>::size_type j = 0;
				j != m_routerTable[i]._nextSet.size(); ++j) {
			int faceID = 0;
			while (faceID < m_adjacentTable.size()) {
				if (m_routerTable[i]._nextSet[j]._role
						!= m_adjacentTable[faceID]._roleID._role
						|| m_routerTable[i]._nextSet[j]._id
								!= m_adjacentTable[faceID]._roleID._id) {
					faceID++;
					continue;
				}
...
int ControlPanel::regist(Role role) {
	 ...
	 */
	msg._bufferLength = sizeof(PacketType) + sizeof(Role) + sizeof(in6_addr)
			+ sizeof(Port) * 4 + 10;
	...
	std::cout << "***********ControlPanel::regist() start***********"
			<< std::endl;
	...
	sleep(5);
	std::cout << "***********ControlPanel::regist() end***************"
			<< std::endl;
...
void ControlPanel::running() {
	...
	sleep(1);
	std::cout << " ControlPanel::running receive adjacent table...: " << id
			<< std::endl;
	printMsg(msg._buffer, msg._msgLength, Ctlr);
	...
	sleep(1);
	std::cout << "ControlPanel::running receive router table...: " << id
			<< std::endl;
	printMsg(msg._buffer, msg._msgLength, Ctlr);
	...
	std::cout << "ControlPanel::running injectTransFaceTableToDataPanel...: "
			<< id << std::endl;
	injectTransFaceTableToDataPanel();

	std::cout << "ControlPanel::running injectFIBToDataPanel...: " << id
			<< std::endl;
